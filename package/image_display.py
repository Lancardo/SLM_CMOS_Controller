...
# Add local libs directory to path for gxipy
...
# Add the libs directory to system path
libs_path = os.path.join(application_path, 'libs')
...
# Import gxipy only if it's available
try:
    import gxipy as gx  # 导入相机SDK
except ImportError:
    # Create a mock gx object for basic functionality without camera
    class MockGx:
        class DeviceManager:
            def __init__(self):
                pass
    ...
    gx = MockGx()
    print("Warning: gxipy not found. Camera functionality will be disabled.")
...
class CameraManager:
    ...
    def capture_image(self):
        ...
        try:
            ...
            file_path = os.path.join(self.save_path, f"capture_{timestamp}.jpg")
...
class ImageDisplayWindow(QWidget):
    def __init__(self, image_list=None, interval=5000, camera_manager=None, enable_camera=False):
        super().__init__()
        # 设置窗口无边框
        ...
        layout.setSpacing(0)  # 移除部件之间的间距
        layout.addWidget(self.image_label)
        self.setLayout(layout)

        # 设置图片列表和计时器
        self.image_list = image_list or []
        self.current_image_index = -1  # 从-1开始，这样第一次next_image会显示索引0的图片
        self.timer = QTimer()
        ...
        # 如果有图片，开始播放
        if self.image_list:
            self.timer.start(interval)  # 设置间隔时间（毫秒）
            self.next_image()  # 立即显示第一张图片
    ...
    def display_image(self, image_path):
        ...
        scaled_pixmap = pixmap.scaled(screen_size, 
                                    Qt.AspectRatioMode.IgnoreAspectRatio,  # 完全填充屏幕
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setGeometry(100, 100, 600, 500)  # 增加窗口高度以适应新控件
        ...
        slm_notice.setWordWrap(True)  # 允许文本换行
        ...
        notice_label.setWordWrap(True)  # 允许文本换行
        ...
        self.interval_input.setText("5000")  # 默认5000毫秒
        ...
        main_layout.addWidget(QLabel("选择显示器:"))
        ...
        bottom_layout.addStretch()  # 添加弹性空间，将签名推到右边
        
        # 创建签名标签
        signature_label = QLabel("@RuiboLan")
        signature_label.setStyleSheet("color: gray; font-style: italic;")  # 设置样式为灰色斜体
    ...
    def select_folder(self):
        ...
        if folder_path:
            ...
            if self.image_list:
                ...
                self.status_label.setText(f"已找到 {len(self.image_list)} 张图片")
            else:
